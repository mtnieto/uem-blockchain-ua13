...
peer chaincode invoke -o orderer0.uem.com:7050  --tls true --cafile $ORDERER_CA -C $CHANNEL_NAME -n cc --peerAddresses $CORE_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE -c '{"Args":["set","hola", "50"]}' --waitForEvent  >&log.txt 
cat log.txt